...
# Build the main algorithm image
./build.sh

MEM_LIMIT="30g"  # Maximum is currently 30g, configurable in your algorithm image settings on grand challenge

# Ensure output folders exist and have write permissions
mkdir -p "$SCRIPTPATH/test/output/images/tumor-lesion-segmentation"
chmod -R 777 "$SCRIPTPATH/test/output"
...
output_dir = '/output/images/tumor-lesion-segmentation'
if not os.listdir(output_dir):
    raise RuntimeError('No output files found!')

file = os.listdir(output_dir)[0]
output = sitk.GetArrayFromImage(sitk.ReadImage(os.path.join(output_dir, file)))
expected_output = sitk.GetArrayFromImage(
    sitk.ReadImage('/expected_output/psma_95b833d46f153cd2_2018-04-16.nii.gz')
)

mse = np.sum((output - expected_output) ** 2)
if mse < 10:
    print('Test passed!')
else:
    print(f'Test failed! MSE={mse}')
...
# Capture the end time and print elapsed time
end_time=$(date +%s)
elapsed_time=$((end_time - start_time))
echo "Total runtime: $elapsed_time seconds"
